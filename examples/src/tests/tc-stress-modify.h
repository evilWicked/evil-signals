...
Tests.add("Stress Modify", [](evil::TestHarness *th, std::string& result)mutable->bool {
	...
	std::vector<evil::Signal<int>>  vecSignals(numSignals);

	//create a collection of slots with associated objects
	const int numSlots = 100;
	std::vector<evil::Slot<int>>  vecSlots(numSlots);
	std::vector<TestClass>  vecObjects(numSlots);

	std::queue<evil::Slot<int> *>  queSlots;
...
//			evil::thread_pool::run([&] {
			...
			while (numTests) {
				...
				case 3: {
					...
					if(x != NULL) {
						evil::Signal<int> *sig=(evil::Signal<int> *)x->signal();
				...
				default: {
					...
					int n = m::rand_int(0, numSignals - 1);
					evil::Signal<int> *sig = &vecSignals[n];
					sig->dispatch(1);
