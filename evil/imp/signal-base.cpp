...
namespace evil {
	...
	void CSignalBase::processAdd(CSlotBase* slot) {
		...
		CPassLock dispatchLock(&mDispatchMutex, CPassLock::BLOCK);
	...
	void CSignalBase::processRemoveAll() {
		...
		std::lock_guard<std::mutex> changeLock(mChangeMutex);
		//if we are dispatching -> block
		CPassLock dispatchLock(&mDispatchMutex, evil::CPassLock::BLOCK);
	...
	void CSignalBase::processSortSlots() {
		...
		CPassLock dispatchLock(&mDispatchMutex, evil::CPassLock::ALLOW);
